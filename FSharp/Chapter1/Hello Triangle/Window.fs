...
type Window(width : int, height : int, title : string) =
    ...
    override __.OnLoad(e : EventArgs) = 
        ...
        GL.BufferData (BufferTarget.ArrayBuffer, (_vertices.Length * sizeof<float32>), _vertices, BufferUsageHint.StaticDraw)
        ...
        // Now, we need to setup how the vertex shader will interpret the VBO data; you can send almost any C datatype (and a few non-C ones too) 
    ...
    override __.OnResize(e : EventArgs) = 
        // When the window gets resized, we have to call GL.Viewport to resize OpenGL's viewport to match the new size.
        // If we don't, the NDC will no longer be correct.
        GL.Viewport (0, 0, width, height)
        base.OnResize (e)
        
    // Now, for cleanup. This isn't technically necessary since F# and OpenGL will clean up all resources automatically when
    // the program closes, but it's very important to know how anyway.
    override __.OnUnload(e : EventArgs) = 
